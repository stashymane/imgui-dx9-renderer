...
use imgui::{
    internal::RawWrapper, BackendFlags, Context, DrawCmd, DrawCmdParams, DrawData, DrawIdx, TextureId,
    Textures,
};
use windows::Win32::Graphics::Direct3D9::{
    IDirect3DBaseTexture9, IDirect3DDevice9, IDirect3DIndexBuffer9, IDirect3DStateBlock9, IDirect3DTexture9,
    IDirect3DVertexBuffer9, D3DBLENDOP_ADD, D3DBLEND_INVSRCALPHA, D3DBLEND_ONE, D3DBLEND_SRCALPHA, D3DCULL_NONE,
    D3DFILL_SOLID, D3DFMT_A8R8G8B8, D3DFMT_INDEX16, D3DFMT_INDEX32, D3DFVF_DIFFUSE,
    D3DFVF_TEX1, D3DFVF_XYZ, D3DLOCKED_RECT, D3DLOCK_DISCARD, D3DPOOL_DEFAULT,
    D3DPT_TRIANGLELIST, D3DRS_ALPHABLENDENABLE, D3DRS_ALPHATESTENABLE, D3DRS_BLENDOP, D3DRS_CLIPPING,
    D3DRS_CULLMODE, D3DRS_DESTBLEND, D3DRS_DESTBLENDALPHA, D3DRS_FILLMODE,
    D3DRS_FOGENABLE, D3DRS_LIGHTING, D3DRS_RANGEFOGENABLE, D3DRS_SCISSORTESTENABLE, D3DRS_SEPARATEALPHABLENDENABLE,
    D3DRS_SHADEMODE, D3DRS_SPECULARENABLE, D3DRS_SRCBLEND, D3DRS_SRCBLENDALPHA, D3DRS_STENCILENABLE,
    D3DRS_ZENABLE, D3DRS_ZWRITEENABLE, D3DSAMP_MAGFILTER, D3DSAMP_MINFILTER, D3DSBT_ALL,
...
/// Reexport of windows_core::Result<T>.
pub type Dx9Result<T> = Result<T>;
...
impl Renderer {
    ...
    pub unsafe fn new(ctx: &mut Context, device: IDirect3DDevice9) -> Result<Self> {
        let t = unsafe { Self::create_font_texture(ctx.fonts(), &device)? };
        ...
        Ok(Renderer {
            vertex_buffer: unsafe { Self::create_vertex_buffer(&device, 0)? },
            index_buffer: unsafe { Self::create_index_buffer(&device, 0)? },
            device,
            font_tex,
    ...
    unsafe fn render_impl(&mut self, draw_data: &DrawData) -> Result<()> {
        ...
        unsafe {
            self.device.SetTexture(0, &self.font_tex)?;
        }
        for draw_list in draw_data.draw_lists() {
            for cmd in draw_list.commands() {
                match cmd {
                    ...
                    DrawCmd::ResetRenderState => unsafe { self.set_render_state(draw_data)? },
...
impl StateBackup {
    unsafe fn backup(device: &IDirect3DDevice9) -> Result<Self> {
        unsafe { device.CreateStateBlock(D3DSBT_ALL).map(StateBackup) }
    }
}
